...
impl SSABackwardIndex {
    pub fn build(ssa: &SSAArtifact, bpa: &BPASTArtifact, bi_blk: &[BIBlkEntry]) -> Vec<BISsaEntry> {
        ...
        for (f_idx, func) in ssa.functions.iter().enumerate() {
            ...
            for var in &func.ssa_records {
                let (ft, lt) = if var.def_stmt != u32::MAX
                    && (var.def_stmt as usize) < bpa.node_count as usize
                {
                    ...
                    if global_bid < bi_blk.len() {
                        (
                            bi_blk[global_bid].first_token_id,
                            bi_blk[global_bid].last_token_id,
                        )
                    } else {
                        (0, 0)
                    }
